...
class Content(TimeFramedModel):
    ...
    def active(self):
        return (self.start is None or self.start <= now()) and (
            self.end is None or self.end >= now()
        )
...
class Video(Content):
    url = models.URLField(
        verbose_name="Stream URL", help_text="Absolute path to video streaming URL."
    )
...
class Display(models.Model):
    ...
    update_interval = models.PositiveIntegerField(
        ...
        help_text="Number of seconds each slide is displayed.",
    )
    ...
    def get_display_url(self):
        return "//%s%s" % (
            ...
            reverse("signage:display_view", args=[self.pk]),
        )

    def get_slides(self):
        return (
            Slide.timeframed.filter(tags__name__in=self.tags.names())
            .order_by("-weight")
            .distinct()
        )
    ...
    def get_video(self):
        return (
            Video.timeframed.filter(tags__name__in=self.tags.names()).distinct().first()
        )
